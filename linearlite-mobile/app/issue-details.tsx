...
import { Comment, issuesMutations, tables } from '@/schema';
...
export default function IssueDetailsScreen() {
  ...
  const parsedComments = comments.map(
    (comment: Comment & { reactions: string }) => ({
      ...comment,
      ...
      <SafeAreaView style={{ flex: 1 }}>
        <ScrollView style={{ flex: 1 }}>
          <View className="px-5">
            {issue.deletedAt ? (
              <View className="flex-row justify-between border my-2 border-zinc-200 dark:border-zinc-700 rounded-md p-2 gap-2">
                <ThemedText style={{ color: 'red' }}>
                  Deleted on{' '}
                  {new Date(issue.deletedAt * 1000).toLocaleDateString(
                    'en-US',
                    { month: 'short', day: 'numeric' },
                  )}{' '}
                  at{' '}
                  {new Date(issue.deletedAt * 1000).toLocaleTimeString(
                    'en-US',
                ...
                <Pressable
                  onPress={() =>
                    store.store.mutate(
                      issuesMutations.restoreIssue({ id: issue.id }),
                    )
                  }
            ...
            <Pressable
              onPress={() => router.push(`/edit-issue?issueId=${issue.id}`)}
            >
              ...
              <View className="flex-row border my-2 border-zinc-200 dark:border-zinc-700 rounded-md p-1 px-2 gap-2">
                ...
                <View className="flex-row items-center gap-2">
                  <Image
                    source={{ uri: issue.assigneePhotoUrl! }}
            ...
            <View className="gap-4 mt-4">
              ...
              {parsedComments.map(
                (
                  ...
                  <View
                    key={comment.id}
                    className="bg-neutral-100 dark:bg-neutral-900 rounded-xl p-2 px-3"
                  >
                    <View className="flex-row items-center gap-2 flex-shrink">
                      ...
                      />
                      <ThemedText
                        className="line-clamp-1 flex-shrink"
